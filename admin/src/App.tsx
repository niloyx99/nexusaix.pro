...
export default function App() {
  ...
  const refreshLicenses = useCallback(async () => {
    ...
    try {
      ...
      if (/401|Invalid admin password|non-JSON|Not connected/i.test(message)) {
        forceLogout(message);
        return;
      }
      setError(message);
  ...
  // Validate existing session against backend on load
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (!isAdminLoggedIn()) {
  ...
  if (bootstrapping) {
    return (
      <div className="relative h-[100dvh] w-full bg-[#0d0f12] text-white/50 flex items-center justify-center text-sm">
  ...
  return (
    <div className="relative h-[100dvh] max-h-[100dvh] w-full font-sans antialiased overflow-hidden bg-[#0d0f12]">
      <GlowBackground />

      <div className="relative h-full w-full bg-[#0d0f12]/75 backdrop-blur-[35px] overflow-hidden flex flex-col">
