...
export default function BottomNavigation({ activeTab, onChangeTab }: BottomNavigationProps) {
  const navItems = [
    ...
    { id: 'news' as TabType, icon: CalendarClock, label: 'News Signal' },
  ...
  return (
    ...
    >
      <div className="h-16 rounded-xl bg-black/10 backdrop-blur-2xl border border-white/[0.06] shadow-[0_10px_30px_rgba(0,0,0,0.5),inset_0_1px_1px_rgba(255,255,255,0.02)] flex items-center justify-around px-2">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className="relative flex flex-col items-center justify-center w-11 h-12 rounded-lg focus:outline-none transition-all duration-300"
            ...
            >
              {isActive && (
                <motion.div
                  ...
                  className="absolute inset-1 rounded-lg bg-white/[0.04] border border-white/[0.08] shadow-[0_0_12px_rgba(255,255,255,0.05)]"
